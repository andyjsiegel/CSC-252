...
fibonacci:
    ...
    beq $s0, $t0, square_print                    # if (fib == 1) goto square_print after printing first two
...
fib_loop:
    ...
    slt $t9, $s0, $t2
    beq $t9, $zero, fib_loop                      # if (t2 <= s0), branch to fib_loop
...
run_check_loop:
    ...
    slt $t8, $t6, $t7           
    bne $t8, $zero, check_ascending               # if (t6 < t7), branch to check_ascending
    
    slt $t8, $t7, $t6
    bne $t8, $zero, check_descending              # if (t6 > t7), branch to check_descending
    ...
    and $t3, $t1, $t2                             # t3 = isAscending (t1) & isDescending (t2)
    ...
    la   $a0, RUN_CHECK_ASC                       # print "Run Check: ASCENDING" 
    syscall

    addi $v0, $zero, 4                            # syscall for print string
    la $a0, RUN_CHECK_DESC                        # print "Run Check: DESCENDING"
    syscall

    j count_words                                 # jump to count_words (next section)
...
check_neither:
    ...
    addi $v0, $zero, 4                            # syscall for print string
    la   $a0, RUN_CHECK_NEITHER                   # print "Run Check: NEITHER"
    syscall
